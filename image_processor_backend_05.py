...
class ImageProcessorManager:
    def __init__(self, params , layer_thickness_file):
        ...
        self.dataframe_data = {  # This will hold the data for the DataFrame
            'segment_name': [],
            'segment_number': [],
            'layer_name': [],
            'percentage': [],
    ...
    def process_images(self):
        ...
        for filename in os.listdir(self.input_dir):
            if filename.lower().endswith(('.bmp', '.jpg', '.png')):
                ...
                for i, segment in enumerate(splitter.segments):
                    ...
                    layer_thickness = self.layer_thickness_data.get(layer_name, -10000)  # Default to -1 if not found
                    black_area_volume_um3 = black_area_um2 * layer_thickness
                    total_volume_um3 = self.segment_area_um2 * layer_thickness

                    # Populate the data structure for DataFrame
        ...
        # Convert accumulated data into a DataFrame for further processing
    ...
    def aggregate_volumes_by_segment(self):
        ...
        volume_aggregation = self.df.groupby('segment_number')['total_volume_um3'].sum().reset_index()
        volume_aggregation.rename(columns={'total_volume_um3': 'total_segment_volume_um3'}, inplace=True)

        # Merge aggregated volume data back into the main DataFrame
        self.df = pd.merge(self.df, volume_aggregation, on='segment_number', how='left')
    ...
    def save_percentages_to_dataframe(self):
        ...
        if not self.df.empty:
            # Save the DataFrame to a CSV file, including the total volume per segment across all layers
            self.df.to_csv(csv_file_path, index=False)
            print("Data saved to CSV file successfully with total volume per segment across all layers.")
        else:
